    ...
    def __getitem__(self, index): 
        while True: 
            try: 
                ...
                video_list = self.data_dict[key]
                data = random.sample(video_list, 2) 
                ...
                for i in range(len(data)): 
                    ...
                    video_reader = decord.VideoReader(data[i]['video_path'], width=self.width, height=self.height)
                    ...
                    if i == 0:
                        ...
                        frames = frames[: self.max_num_frames-1]
                    ...
                    text += "[VIDEO" + str(i+1) + "] " + data[i]['text']
            
                assert (frames_num) % 4 == 1
            
                if len(video_clips) == len(data):
                    break 
            except Exception as e:
                index = (index + 1) % len(self.key_list) 
        ...
        # print(video_clips, video_clips[0].size(), text) 
        # Training transforms
        # frames = (frames - 127.5) / 127.5 
        frames = torch.cat(video_clips, dim=0) 
        ...
        return frames.contiguous(), text_input_ids #, text
